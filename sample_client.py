...
class SampleApplication(stack.ApplicationListener):
    ...
    def onRequest(self, peer, request):
        print "Got request"
        x = peer.stack.createAnswer(request)
        peer.stack.sendByPeer(peer, x)
...
class SamplePeerListener(stack.PeerListener):
    ...
    def connected(self, peer):
        ...
        gy = peer.stack.getDictionary("base")
        req = peer.stack.createRequest(16777256, 272, True)
...
def main():
    dstack = stack.Stack()
    dstack.loadDictionary("base", "dictionary.xml")
    dstack.identity = "msk.cscm.inlinetelecom.ru"
    dstack.realm = "cscm.inlinetelecom.ru"

    app = SampleApplication()
    dstack.registerAuthApplication(app, 0, 16777256)
    dstack.registerAcctApplication(app, 0, 16777256)

    peer_listener = SamplePeerListener()
    dstack.registerPeerListener(peer_listener)

    dstack.registerPeerIO(SamplePeerIO())
    dstack.clientV4Add("127.0.0.1", 3868)
